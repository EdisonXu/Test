...
public class NbiEventSendTask implements Runnable{
    ...
    private void handleEvent(NbiEventCache.STATUS status, MyEventRetryTO event)
    {
        switch(status)
        {
        case OVERLOAD:
    ...
    private void handleNext(MyEventRetryTO old) {
    	/*
    	 ...
    	 */
    	if(old==null || old.getSessionId()==null) return;
    	handleEvent(NbiEventCache.getStatus(), old);
    }
    
    private void handleOK(MyEventRetryTO event)
    {
        if(event==null) return;
        sendEvent(event);
    }
    
    private void handleOverload(MyEventRetryTO event) {
        if(event==null) return;
...
/*            System.out.println(Thread.currentThread().getName() +
                    " Discard NBI event '" + event.toString() + "', current queue size: " 
                            + NbiEventCache.size());
*/            System.out.println(Thread.currentThread().getName() +
                    " Discard NBI event ' Id=" + event.getId() + ", eMBMSSessionId=" + event.getSessionId() + "', current queue size: " 
                            + NbiEventCache.size());
    ...
    private void sendEvent(MyEventRetryTO event) {
        doSend(event);
    ...
    private void doSend(MyEventRetryTO event){
        // send event
        MyEventHttpSender bmscEventSender = NbiEventCacheUtil.getSender();
        ...
        while (true) {
            ...
            if (retrytimes > 0) {
                ...
                NbiEventCacheUtil.updateMyEventInDB(event);
            }
    ...
    private void removeFromDb(MyEventRetryTO event) {
        NbiEventCacheUtil.removeMyEventInDB(event);
    }
    
    private void removeFromDb(List<MyEventRetryTO> list) {
        for(MyEventRetryTO e:list)
            removeFromDb(e);
    }
